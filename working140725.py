...
2. CHAT - User wants chat, short ans, perticular result, calculation
...
"""
            ...
            # Ensure query has proper structure - only wrap if it's a bare query clause
            # Don't wrap if it already contains top-level fields like aggs, size, from, _source
            if not any(key in query_json for key in ['aggs', 'size', 'from', '_source', 'sort']):
                if 'query' not in query_json:
                    query_json = {"query": query_json}

            # Add size limit if not present and no aggregations
    ...
    def respond(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            if state['intent'] == 'table':
                ...
                # Format raw data for chat context
                formatted_data = self._format_raw_data(state['raw_data'])
                state['summarized_data'] = formatted_data
    ...
    def _format_raw_data(self, raw_data: List[Dict]) -> str:
        """Format raw data for chat context without summarization"""
        try:
            if not raw_data:
            ...
            # Check if this looks like aggregation data (has 'key' and 'doc_count' fields)
            is_aggregation = any('key' in rec and 'doc_count' in rec for rec in raw_data)

            if is_aggregation:
                # Format aggregation data as JSON for clarity
                formatted = []
                formatted.append("RAW AGGREGATION DATA:")
                formatted.append(f"Total records: {len(raw_data)}")
                formatted.append("\nData:")
                formatted.append(json.dumps(raw_data, indent=2))

                # Add simple breakdown for context
                formatted.append("\n\nSimple breakdown:")
                for i, rec in enumerate(raw_data):
                    if 'key' in rec:
                        formatted.append(f"Record {i + 1}: {rec}")
                    else:
                        formatted.append(f"Total/Summary: {rec}")

                return "\n".join(formatted)

            else:
                # Handle regular data (flatten if needed)
                if any(any(isinstance(v, dict) for v in rec.values()) for rec in raw_data):
                    flat_data = [flatten(rec) for rec in raw_data]
                else:
                    flat_data = raw_data
                ...
                formatted.append(f"Total records: {len(flat_data)}")
                formatted.append("\nData:")
                formatted.append(json.dumps(flat_data, indent=2))
...
2. Use banking terminology appropriately
3. keep it short and simple
4. ONLY USE PROVIDED CONTEXT

Response:"""

            response = self.ollama_llm.invoke(prompt)

            # Print Ollama response for debugging
            print(f"\n\n===== OLLAMA RESPONSE =====\n{response}\n==========================\n\n")
...
def main():
    ...
    try:
        ...
        # If session state is not ready, initialize with defaults
        st.session_state = {
            'agent': None,
            'agent_ready': False,
            'query_history': [],
    ...
    try:
        ...
        if result:
            if not result.get('success', False):
                ...
                # Debug: Print intent and response info
                st.write(f"**Debug - Intent:** {intent}")
                st.write(f"**Debug - Final Response:** {result.get('final_response', 'NO RESPONSE')}")
                st.write(f"**Debug - Raw Data Length:** {len(result.get('raw_data', []))}")
